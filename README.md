...
进入跌倒危险状态后不需要再按 `Enter`。系统会自动连续监听“我没事”“我还好”等安全确认，解除危险状态后再回到普通按键说话模式。

危险状态下的自动监听使用轻量 VAD：先等待人声出现，检测到一小段静音后立刻识别，所以通常不用等完整录音时长。`--danger-record-seconds` 表示最长等待/最长录音窗口，不是每次固定等待时间。为了避免把系统自己的提示音录进去，危险状态默认会在播报后等待 2 秒再开始听。

语音热词在 `voice/hotwords.txt` 里维护，格式可以是：
...
- 静态检测判断为 `fall`，或者动态序列检测判断为跌倒，任意一个命中就立即进入 `FALL`；
- 动态序列分支要求当前帧至少检测到左髋或右髋，只有脸、肩或手臂时不会触发动态跌倒判断；
- 静态分支优先要求左髋或右髋可见；如果真实躺倒导致姿态关键点检测失败，则允许高置信、足够大且横向摊开的静态 `fall` 框作为兜底触发；
- 一旦进入危险状态，语音助手会询问“我检测到可能跌倒。你还好吗？如果没事，请说我没事，或者我还好。”；
- 这句提示会优先播放 `assets/audio/fall_prompt.wav`，如果没有这个录音文件，会自动用 TTS 生成 `.cache/audio/fall_prompt.wav`，之后直接播放缓存音频；
- 只有听到“我没事”“我还好”“没关系”“没问题”等确认词后，才解除危险状态；
- 解除后默认有 3 秒保护时间，避免同一帧姿态马上把状态重新拉回 `FALL`。
- 默认平时仍然需要按 `Enter` 触发录音；进入 `FALL` 后会自动使用 VAD 连续监听，直到解除危险状态。
...
- `--voice-hotwords-file voice/hotwords.txt`：指定语音热词表；
- `--no-vad`：关闭危险状态下的自动监听 VAD，改回固定时长录音；
- `--danger-record-seconds 3.0`：危险状态自动监听的最长等待/录音窗口；
- `--danger-listen-delay 2.0`：危险提示播报后延迟多久再开始自动监听；
- `--danger-prompt-interval 20.0`：危险状态下重复提示的间隔；
- `--no-tts`：只打印文字，不播报；
